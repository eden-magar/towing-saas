...
-- Adds vehicle_type to search_calendar_tows return set (first tow_vehicle by order_index,
-- same selection as plate). Does not change match logic, existing columns, or function
-- volatility/security (sql STABLE, invoker).
...
COMMENT ON FUNCTION public.search_calendar_tows(uuid, text, int) IS
  ...
  'addresses, and driver name. Returns one row per tow with order numbers, '
  'pickup/dropoff leg addresses, and first vehicle type by order_index, ordered by scheduled_at desc.';
