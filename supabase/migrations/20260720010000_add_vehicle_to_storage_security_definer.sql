-- Driver JWT cannot INSERT stored_vehicles / storage_history under RLS when
-- add_vehicle_to_storage runs as SECURITY INVOKER (get_my_company_id() fails
-- for drivers). release_vehicle_from_storage already uses SECURITY DEFINER.
-- Restore DEFINER on the 12-arg add overload only — body otherwise unchanged.
