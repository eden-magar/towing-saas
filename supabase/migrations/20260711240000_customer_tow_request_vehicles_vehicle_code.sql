-- Portal request vehicles: company-internal vehicle code (קוד רכב).
-- Updates create_full_customer_tow_request to persist vehicle_code from jsonb.
-- No backfill.

ALTER TABLE public.customer_tow_request_vehicles
  ADD COLUMN IF NOT EXISTS vehicle_code text NULL;

COMMENT ON COLUMN public.customer_tow_request_vehicles.vehicle_code IS
  'Optional company-internal vehicle code from portal entry.';

-- Full RPC body mirrors 20260711230000 (+ vehicle_code on vehicles).
