-- Portal request vehicles: persist stored_vehicle_id from jsonb into
-- customer_tow_request_vehicles (column added in 20260724060100).
-- MUST be applied in production — column exists; RPC does not write it yet.
-- Full RPC body mirrors 20260711240000 (+ stored_vehicle_id on vehicles).
  ...
  FOR i IN 0 .. n_vehicles - 1 LOOP
    ...
    INSERT INTO public.customer_tow_request_vehicles (
      ...
      CASE
        ...
        ELSE NULL
      END,
      NULLIF(trim(v_vehicle->>'vehicle_code'), ''),
      CASE
        WHEN v_vehicle->>'stored_vehicle_id' IS NOT NULL
             AND trim(v_vehicle->>'stored_vehicle_id') <> ''
        THEN (v_vehicle->>'stored_vehicle_id')::uuid
        ELSE NULL
      END,
