-- storage_history.performed_by FK → users(id). Expo often passes drivers.id
-- (or another non-users uuid) as p_performed_by, which raised 23503 and rolled
-- back the entire add/release. Resolve to a real users.id or NULL so the
-- warehouse write still commits. Keep manual_action_items logging unchanged.
...
BEGIN
  ...
  v_safe_performed_by := (SELECT id FROM users WHERE id = p_performed_by);

  INSERT INTO storage_history (stored_vehicle_id, action, tow_id, performed_by, notes)
  VALUES (v_stored_vehicle_id, 'in', p_tow_id, v_safe_performed_by, p_notes);
...
BEGIN
  ...
  INSERT INTO storage_history (stored_vehicle_id, action, tow_id, performed_by, notes)
  VALUES (p_stored_vehicle_id, 'out', p_tow_id, v_safe_performed_by, p_notes);
